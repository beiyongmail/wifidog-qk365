...
main_loop(void)
{
	...
	/* save the pid file if needed 保存wifidog的进程号*/
    ...
    /* 添加关键路径对应的回调函数 */
    /**
     * 回调函数主要用于根据用户http报文执行不同的操作，其原理就是分析http报文
     * 请求中有没有关键路径，若有，则执行关键路径对应的回调函数，若没有，则返
     * 回一个重定向到认证服务器的包给用户。
     */
    ...
    //循环等待客户端连接
    //（使用socket绑定2060端口并监听，实际上在建立新的iptables路由表规则时会将网关的80端口重定向到2060端口）
...
gw_main(int argc, char **argv)
{
    ...
    s_config *config = config_get_config();     //返回全局变量config结构体的地址
    ...
    if (restart_orig_pid) {
        /* 用于restart，如果有已运行的wifidog，先会kill它
