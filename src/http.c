...
/**
 * 此段代码表示wifidog是如何通过http 404回调函数实现客户端重定向了，
 * 实际上就是在404回调函数中封装了一个307状态的http报头，http的307状态
 * 在http协议中就是用于重定向的，封装完成后通过已经与客户端连接的socket返回给客户端。步骤流程为
...
/**
 * 此段表明当客户端已经在认证服务器确认登陆，认证服务器将客户端重新重定向回网关，
 * 并在重定向包中包含关键路径"/wifidog/auth"和token，认证服务器所执行的操作。
 */
void
